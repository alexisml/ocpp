...
def _resolve_central_system(hass: HomeAssistant, devid: str):
    ...
    *devid* is matched against both the HA charger id (``cpid``) and the raw
    OCPP charger id (``cp_id``) for every active central system.  If no match
    is found a :class:`HomeAssistantError` is raised so the caller gets an
    explicit failure instead of silently routing to the wrong charger.
    """
    for cs in _iter_central_systems(hass):
